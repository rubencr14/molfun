...
from molfun.data.splits import random_split
...
def parse_args():
    ...
    p.add_argument("--max-seq-len", type=int, default=384,
                    help="Max sequence length for featurizer (default: 384)")
...
def main():
    ...
    print(f"[1/7] Fetching up to {args.max_structures} human kinase structures...")
    ...
    print(f"      Downloaded {len(paths)} structures to {args.data_dir}")
    ...
    train_ds, val_ds, test_ds = random_split(dataset, ratios=[0.8, 0.1, 0.1])
    ...
    print(f"[6/7] Saving checkpoint to {args.output_dir}...")
