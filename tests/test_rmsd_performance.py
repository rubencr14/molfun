...
def test_rmsd_performance():
    ...
    start = time.time()
    rmsds_molfun = molfun.rmsd(0, superposition=True)[:n_frames]
    torch.cuda.synchronize() if torch.cuda.is_available() else None
    molfun_time = time.time() - start
    print(f"RMSD calculation time ({n_frames} frames): {molfun_time:.3f}s ({molfun_time*1000/n_frames:.3f}ms/frame)")
    ...
    print(f"RMSD calculation time ({n_frames} frames): {traj_time:.3f}s ({traj_time*1000/n_frames:.3f}ms/frame)")
    ...
    print(f"RMSD calculation time ({n_frames} frames): {mda_time:.3f}s ({mda_time*1000/n_frames:.3f}ms/frame)")
