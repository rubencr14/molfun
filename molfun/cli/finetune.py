...
def structure(
    pdbs: Annotated[
        Path, typer.Argument(help="Directory of PDB files, or a .txt file listing their paths.")
    ],
    ...
    val_frac: Annotated[
        float, typer.Option(help="Fraction of PDBs held out for validation.")
    ] = 0.2,
    seed: Annotated[int, typer.Option()] = 42,
    weights: Annotated[Path, typer.Option(help="Pre-trained OpenFold weights.")] = _DEFAULT_WEIGHTS,
    output: Annotated[Path, typer.Option(help="Directory for checkpoint and history.")] = Path(
        "runs/structure/checkpoint"
    ),
):
    ...
    class _PDBDataset(Dataset):
        def __len__(self):
            return len(pdb_paths)

        def __getitem__(self, i):
            return featurizer.from_pdb(pdb_paths[i], chain_id="A")
...
def affinity(
    ...
    if demo:
        ...
        train_loader = DataLoader(
            DummyOpenFoldDataset(n - 2, seq_len=32, n_msa=4), batch_size=1, collate_fn=_collate
        )
        val_loader = DataLoader(
            DummyOpenFoldDataset(2, seq_len=32, n_msa=4), batch_size=1, collate_fn=_collate
        )
        ...
        train_ds, val_ds, _ = DataSplitter.random(
            dataset, val_frac=val_frac, test_frac=0.1, seed=seed
        )
        train_loader = DataLoader(
            train_ds, batch_size=1, shuffle=True, collate_fn=AffinityDataset.collate_fn
        )
        val_loader = DataLoader(
            val_ds, batch_size=1, shuffle=False, collate_fn=AffinityDataset.collate_fn
        )
