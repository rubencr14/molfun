...
def agent(
    ...
    model: Annotated[
        str, typer.Option(help="Model name (e.g. gpt-4o-mini, qwen2.5-7b).")
    ] = "gpt-4o-mini",
    api_key: Annotated[
        str | None, typer.Option(envvar="OPENAI_API_KEY", help="API key (or set env var).")
    ] = None,
    base_url: Annotated[str | None, typer.Option(help="API base URL (for LM Studio/vLLM).")] = None,
    port: Annotated[int, typer.Option(help="Port for local servers (LM Studio/Ollama).")] = 1234,
    pdbs: Annotated[Path, typer.Option(help="Directory with PDB/CIF structure files.")] = Path(
        "data/structures"
    ),
    ...
    memory_dir: Annotated[
        Path, typer.Option(help="Directory to persist experiment memory.")
    ] = Path("runs/agent/memory"),
    tracker: Annotated[
        str | None, typer.Option(help="Tracker: console, wandb, comet, mlflow.")
    ] = None,
...
def _build_data(pdbs: Path, data_csv: Path | None, max_seq_len: int, device: str):
    import torch
    ...
    if data_csv and data_csv.exists():
        ...
        train_ds, val_ds, test_ds = DataSplitter.random(
            dataset, val_frac=0.15, test_frac=0.15, seed=42
        )
