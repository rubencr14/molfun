...
class PDBFetcher:
    ...
    def __init__(self, cache_dir: Optional[str] = None, fmt: str = "cif"):
        """
        Args:
            cache_dir: Directory to cache downloaded files. Default: ~/.molfun/pdb_cache
    ...
    def fetch(self, pdb_ids: list[str]) -> list[Path]:
        """
        Download structures by PDB ID.

        Returns list of local file paths (same order as input).
    ...
    def list_cached(self) -> list[Path]:
        ...
        return sorted(self.cache_dir.glob(f"*.{self.fmt}.gz")) + sorted(
            self.cache_dir.glob(f"*.{self.fmt}")
        )
    ...
    def _cached_path(self, pdb_id: str) -> Path:
        return self.cache_dir / f"{pdb_id}.{self.fmt}"

    def _download(self, pdb_id: str, dest: Path) -> None:
