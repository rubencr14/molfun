...
from molfun.kernels.analysis.contact_map_atoms import contact_map_atoms_bitpack
from molfun.kernels.analysis.rmsd import rmsd_aligned_batch_fused, rmsd_batch_triton, rmsd_triton
...
class MolfunAnalysis:
    ...
    def get_coords(self, frame: int | None = None) -> torch.Tensor:
    ...
    def contact_map(self, cutoff: float, frame: int | None = None) -> torch.Tensor:
    ...
    def rmsd(
        self, ref_frame: int = 0, frame: int | None = None, superposition: bool = True
...
class TrajAnalysis:
    ...
    def contact_map(self, cutoff: float, frame: int | None = None) -> np.ndarray:
    ...
    def rmsd(self, ref_frame: int = 0, frame: int | None = None) -> Union[float, np.ndarray]:
...
class AnalysisMD:
    ...
    def get_coords(self, frame: int | None = None) -> np.ndarray:
        ...
        if frame is not None:
            ...
            coords = []
            for _ts in self.universe.trajectory:
    ...
    def rmsd(
        ...
        if frame is not None:
            ...
            rmsd_analysis = MDA_RMSD(
                self.universe, self.universe, select=sel_str, ref_frame=ref_frame
            )
            rmsd_analysis.run()
