...
class FullFinetune(FinetuneStrategy):
    ...
    Each evoformer block gets: lr * layer_lr_decay^(N - block_index).
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.lr_embedder = lr_embedder or self.lr * self.layer_lr_decay ** 48
    ...
    def param_groups(self, model) -> list[dict]:
        ...
        # Input embedder — deepest, lowest LR
        embedder = getattr(model.adapter.model, "input_embedder", None)
        if embedder is not None:
            params = [p for p in embedder.parameters() if p.requires_grad]
            seen.update(id(p) for p in params)
            if params:
                groups.append({"params": params, "lr": self.lr_embedder})

        # Evoformer blocks — layer-wise decay
        ...
        # Remaining trunk params (template embedder, aux heads, etc.)
        remaining = [
            p for p in model.adapter.model.parameters()
            if p.requires_grad and id(p) not in seen
        ]
        if remaining:
            groups.append({"params": remaining, "lr": self.lr})

        # Task head — highest LR
