...
def rmsd_batch_kernel_1d(
    ...
    for start in range(0, N, BLOCK):
        ...
        rx = tl.load(ref_ptr + i * stride_ref0 + 0 * stride_ref1, mask=mask, other=0.0).to(
            tl.float32
        )
        ry = tl.load(ref_ptr + i * stride_ref0 + 1 * stride_ref1, mask=mask, other=0.0).to(
            tl.float32
        )
        rz = tl.load(ref_ptr + i * stride_ref0 + 2 * stride_ref1, mask=mask, other=0.0).to(
            tl.float32
        )
        ...
        bx = tl.load(
            batch_ptr + m * stride_b0 + i * stride_b1 + 0 * stride_b2, mask=mask, other=0.0
        ).to(tl.float32)
        by = tl.load(
            batch_ptr + m * stride_b0 + i * stride_b1 + 1 * stride_b2, mask=mask, other=0.0
        ).to(tl.float32)
        bz = tl.load(
            batch_ptr + m * stride_b0 + i * stride_b1 + 2 * stride_b2, mask=mask, other=0.0
        ).to(tl.float32)
...
def rmsd_batch_triton(
    coords_A: torch.Tensor, coords_B_batch: torch.Tensor, block: int = 1024
) -> torch.Tensor:
    """
    ...
    assert coords_B_batch.dim() == 3 and coords_B_batch.shape[2] == 3, (
        "coords_B_batch must be (M, N, 3)"
    )
...
def kabsch_stats_batch_kernel(
    ...
    stride_t1: tl.constexpr,
    stride_t2: tl.constexpr,
    ...
    for start in range(0, N, BLOCK):
        ...
        bx = tl.load(
            traj_ptr + m * stride_t0 + i * stride_t1 + 0 * stride_t2, mask=mask, other=0.0
        ).to(tl.float32)
        by = tl.load(
            traj_ptr + m * stride_t0 + i * stride_t1 + 1 * stride_t2, mask=mask, other=0.0
        ).to(tl.float32)
        bz = tl.load(
            traj_ptr + m * stride_t0 + i * stride_t1 + 2 * stride_t2, mask=mask, other=0.0
        ).to(tl.float32)
...
def kabsch_stats_batch(ref: torch.Tensor, traj: torch.Tensor, block: int = 2048) -> torch.Tensor:
    ...
    kabsch_stats_batch_kernel[grid](
        ...
        stride_r0=ref.stride(0),
        stride_r1=ref.stride(1),
        stride_t0=traj.stride(0),
        stride_t1=traj.stride(1),
        stride_t2=traj.stride(2),
...
def rmsd_aligned_from_stats(
    ...
    rmsd_sq = torch.where(rmsd_sq < relative_threshold, torch.zeros_like(rmsd_sq), rmsd_sq)
...
def rmsd_aligned_batch_fused(
    ref: torch.Tensor, traj: torch.Tensor, block: int = 2048
) -> torch.Tensor:
    """
