...
def run_benchmark(topology: str, trajectory: str, return_results: bool = False):
    ...
    print(
        f"  TrajAnalysis:   {t_traj:8.3f}ms | Contacts: {n_contacts_traj:>8,} | Speedup: {t_traj / t_molfun:.2f}x"
    )
    print(
        f"  AnalysisMD:     {t_mda:8.3f}ms | Contacts: {n_contacts_mda:>8,} | Speedup: {t_mda / t_molfun:.2f}x"
    )
        ...
        results.append(
            {
                ...
                "speedup": float(round(to_native_type(t_traj) / to_native_type(t_molfun), 2))
                if t_molfun > 0
                else None,
                "max_diff": float(
                    abs(to_native_type(n_contacts_molfun) - to_native_type(n_contacts_traj))
                ),
                "mean_diff": float(
                    abs(to_native_type(n_contacts_molfun) - to_native_type(n_contacts_traj))
                ),
                "metadata": {
                    ...
                    "speedup_vs_mdtraj": float(
                        round(to_native_type(t_traj) / to_native_type(t_molfun), 2)
                    )
                    if t_molfun > 0
                    else None,
                    "speedup_vs_mdanalysis": float(
                        round(to_native_type(t_mda) / to_native_type(t_molfun), 2)
                    )
                    if t_molfun > 0
                    else None,
                },
    ...
    print(
        f"  TrajAnalysis:   {t_traj:8.3f}ms | RMSD: {rmsd_traj:.4f}Å | Speedup: {t_traj / t_molfun:.2f}x | Match: {'✓' if match_traj else '✗'} (diff: {diff_traj:.2e}Å)"
    )
    print(
        f"  AnalysisMD:     {t_mda:8.3f}ms | RMSD: {rmsd_mda:.4f}Å | Speedup: {t_mda / t_molfun:.2f}x | Match: {'✓' if match_mda else '✗'} (diff: {diff_mda:.2e}Å)"
    )
    ...
    if not match_traj or not match_mda:
        print(
            "  ⚠️  RMSD values differ (small differences expected due to numerical precision in superposition)"
        )
    ...
    min(100, molfun.n_frames)  # Display only first 100 for readability
    ...
    print(
        f"  TrajAnalysis:   {t_traj_cm:8.3f}ms | {t_traj_cm / n_frames_to_test:.3f}ms/frame | Speedup: {t_traj_cm / t_molfun_cm:.2f}x"
    )
    print(
        f"  AnalysisMD:     {t_mda_cm:8.3f}ms | {t_mda_cm / n_frames_to_test:.3f}ms/frame | Speedup: {t_mda_cm / t_molfun_cm:.2f}x"
    )
        ...
        results.append(
            {
                ...
                "speedup": float(round(to_native_type(t_traj_cm) / to_native_type(t_molfun_cm), 2))
                if t_molfun_cm > 0
                else None,
                ...
                "metadata": {
                    ...
                    "molfun_ms_per_frame": float(
                        round(to_native_type(t_molfun_cm) / to_native_type(n_frames_to_test), 4)
                    ),
                    "mdtraj_ms_per_frame": float(
                        round(to_native_type(t_traj_cm) / to_native_type(n_frames_to_test), 4)
                    ),
                    "mdanalysis_ms_per_frame": float(
                        round(to_native_type(t_mda_cm) / to_native_type(n_frames_to_test), 4)
                    ),
                    "speedup_vs_mdtraj": float(
                        round(to_native_type(t_traj_cm) / to_native_type(t_molfun_cm), 2)
                    )
                    if t_molfun_cm > 0
                    else None,
                    "speedup_vs_mdanalysis": float(
                        round(to_native_type(t_mda_cm) / to_native_type(t_molfun_cm), 2)
                    )
                    if t_molfun_cm > 0
                    else None,
                },
    ...
    print("  Note: MolfunAnalysis.rmsd() calculates all frames in a single GPU batch operation")
    ...
    match_traj = (
        max_diff_traj < 1e-2
    )  # Tolerance for superposition (can have small numerical differences)
    ...
    print(
        f"  MolfunAnalysis: {t_molfun_rmsd:8.3f}ms | {t_molfun_rmsd / n_frames_to_test:.3f}ms/frame"
    )
    print(
        f"  TrajAnalysis:   {t_traj_rmsd:8.3f}ms | {t_traj_rmsd / n_frames_to_test:.3f}ms/frame | Speedup: {t_traj_rmsd / t_molfun_rmsd:.2f}x | Match: {'✓' if match_traj else '✗'} (max_diff: {max_diff_traj:.2e}Å)"
    )
    print(
        f"  AnalysisMD:     {t_mda_rmsd:8.3f}ms | {t_mda_rmsd / n_frames_to_test:.3f}ms/frame | Speedup: {t_mda_rmsd / t_molfun_rmsd:.2f}x | Match: {'✓' if match_mda else '✗'} (max_diff: {max_diff_mda:.2e}Å)"
    )
        ...
        results.append(
            {
                ...
                "speedup": float(
                    round(to_native_type(t_traj_rmsd) / to_native_type(t_molfun_rmsd), 2)
                )
                if t_molfun_rmsd > 0
                else None,
                "max_diff": float(max(to_native_type(max_diff_traj), to_native_type(max_diff_mda))),
                "mean_diff": float(
                    (to_native_type(mean_diff_traj) + to_native_type(mean_diff_mda)) / 2
                ),
                "metadata": {
                    ...
                    "molfun_ms_per_frame": float(
                        round(to_native_type(t_molfun_rmsd) / to_native_type(n_frames_to_test), 4)
                    ),
                    "mdtraj_ms_per_frame": float(
                        round(to_native_type(t_traj_rmsd) / to_native_type(n_frames_to_test), 4)
                    ),
                    "mdanalysis_ms_per_frame": float(
                        round(to_native_type(t_mda_rmsd) / to_native_type(n_frames_to_test), 4)
                    ),
                    ...
                    "speedup_vs_mdanalysis": float(
                        round(to_native_type(t_mda_rmsd) / to_native_type(t_molfun_rmsd), 2)
                    )
                    if t_molfun_rmsd > 0
                    else None,
                },
    ...
    if not match_traj or not match_mda:
        ...
        if not match_mda:
            print(
                f"     vs MDAnalysis: max_diff={max_diff_mda:.6f}Å, mean_diff={mean_diff_mda:.6f}Å"
            )
            print(f"     First 5 values - Molfun:    {rmsds_molfun_np[:5]}")
    ...
    print("Summary")
    print("=" * 100)
    print("MolfunAnalysis (Triton kernels) performance:")
    print(
        f"  - Contact maps: {t_traj_cm / t_molfun_cm:.1f}x faster than MDTraj, {t_mda_cm / t_molfun_cm:.1f}x faster than MDAnalysis"
    )
    print(
        f"  - RMSD: {t_traj_rmsd / t_molfun_rmsd:.2f}x vs MDTraj, {t_mda_rmsd / t_molfun_rmsd:.1f}x vs MDAnalysis"
    )
...
def run_benchmark_api():
    ...
    TOPOLOGY_PATH = (
        "/home/rubencr/Escritorio/projects/nomosis/MD_FILES_IKER/MD_LEISH_DPPE_Reduced.prmtop"
    )
    TRAJECTORY_PATH = (
        "/home/rubencr/Escritorio/projects/nomosis/MD_FILES_IKER/MD_LEISH_DPPE_Reduced.dcd"
    )
