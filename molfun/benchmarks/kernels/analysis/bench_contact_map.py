...
def count_bits_packed(packed: torch.Tensor) -> int:
    ...
    popcount_lut = torch.tensor(
        [bin(i).count("1") for i in range(256)], device=packed.device, dtype=torch.int32
    )
...
def run_benchmark() -> list[dict[str, Any]]:
    ...
    for N, cutoff in cases:
        ...
        print(
            f"N={N:5d} cut={cutoff:.1f}Å | PyTorch: {t_pytorch:8.3f}ms | "
...
def main():
    ...
    print("=" * 110)
    print(
        f"{'Case':<22} | {'PyTorch':>10} | {'Triton':>10} | {'Speedup':>8} | "
    ...
    for result in results:
        ...
        print(
            f"{case:<22} | {result['baseline_time_ms']:10.3f} | {result['triton_time_ms']:10.3f} | "
