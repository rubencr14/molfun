...
def run_benchmark() -> list[dict[str, Any]]:
    ...
    for N in cases:
        ...
        t_torch = time_it_cuda(lambda: torch.cdist(coords, coords), iters=50, warmup=10)
        ...
        t_triton = time_it_cuda(lambda: pairwise_distances_triton(coords), iters=50, warmup=10)
        ...
        print(
            f"N={N:4d} | torch: {t_torch:6.3f} ms | triton: {t_triton:6.3f} ms | "
...
def main():
    ...
    print("=" * 80)
    print(
        f"{'N':>6} | {'PyTorch (ms)':>12} | {'Triton (ms)':>12} | {'Speedup':>8} | {'Max Diff':>12}"
    )
    print("-" * 80)
    ...
    for result in results:
        ...
        print(
            f"{meta['N']:6d} | {result['baseline_time_ms']:12.3f} | "
